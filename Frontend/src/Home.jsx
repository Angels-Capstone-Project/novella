...
const Home = ({ userId }) => {
  ...
  useEffect(() => {
    const fetchTopPicks = async () => {
      try {
        const res = await axios.get(`/top-picks/${userId}`);
    ...
    const fetchTopUS = async () => {
      try {
        const res = await axios.get("/top-us");
        setTopUS(res.data);
      } catch (err) {
      ...
      try {
        const results = await Promise.all(
          genres.map(async (genre) => {
            const res = await axios.get(`/genre/${genre}`);
  ...
  return (
    <div style={{ padding: "20px" }}>
      {renderStoryList("Top Picks for You", Array.isArray(topPicks) ? topPicks:[])}
      {renderStoryList("Top 10 in the U.S.", Array.isArray(topUS) ? topUS:[], true)}
      {Object.keys(trendingByGenre).map((genre) =>
        renderStoryList(`Trending in ${genre}`, Array.isArray(trendingByGenre[genre]) ? trendingByGenre[genre]:[])
      )}
    </div>
  );
};
